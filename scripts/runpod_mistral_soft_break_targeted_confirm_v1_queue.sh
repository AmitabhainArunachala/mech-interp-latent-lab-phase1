...
OUTPUT_STEM="${OUTPUT_STEM:-mistral_soft_break_targeted_confirm_v1}"
QUEUE_GROUP="${QUEUE_GROUP:-mistral_soft_break_targeted_confirm_v1}"
EXPERIMENT_ID="${EXPERIMENT_ID:-mistral_soft_break_targeted_confirm_v1}"
CLAIM_ID="${CLAIM_ID:-MISTRAL_SOFT_BREAK_TARGETED_CONFIRM_V1}"
NOTES="${NOTES:-Higher-power confirm for best soft selective break handle.}"
SCALE="${SCALE:?SCALE is required}"
TOKEN_WINDOW="${TOKEN_WINDOW:?TOKEN_WINDOW is required}"
CONDITION_NAME="${CONDITION_NAME:?CONDITION_NAME is required}"
GENERATION_SEEDS="${GENERATION_SEEDS:-101,202,303,404,505,606,707,808,909,1001,1102,1203}"
...
run_step "$EXPERIMENT_ID" \
  "$PYTHON_BIN" scripts/mistral_soft_break_targeted_confirm_v1.py \
  --device cuda \
  --out-dir "$RUN_OUT" \
...
"$PYTHON_BIN" -m src.utils.research_os result-upsert \
  ...
  --config-path "scripts/mistral_soft_break_targeted_confirm_v1.py --condition-name $CONDITION_NAME --scale $SCALE --token-window $TOKEN_WINDOW --generation-seeds $GENERATION_SEEDS --train-per-group $TRAIN_PER_GROUP --test-per-group $TEST_PER_GROUP --max-new-tokens $MAX_NEW_TOKENS --temperature $TEMPERATURE --top-p $TOP_P" \
  --prompt-contract mistral_soft_break_targeted_confirm \
  --metric-path "higher-power confirm for best anti-late break handle on recursive and baseline prompts + BT_ART + MALFORMED" \
  --claim-id "$CLAIM_ID"
