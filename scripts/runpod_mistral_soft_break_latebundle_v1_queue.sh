...
OUTPUT_STEM="${OUTPUT_STEM:-mistral_soft_break_latebundle_v1}"
QUEUE_GROUP="${QUEUE_GROUP:-mistral_soft_break_latebundle_v1}"
EXPERIMENT_ID="${EXPERIMENT_ID:-mistral_soft_break_latebundle_v1}"
CLAIM_ID="${CLAIM_ID:-MISTRAL_SOFT_BREAK_LATEBUNDLE_V1}"
NOTES="${NOTES:-Soft anti-steer break test on recursive vs baseline prompts with late-stack counter-interventions.}"
...
run_step "$EXPERIMENT_ID" \
  "$PYTHON_BIN" scripts/mistral_soft_break_latebundle_v1.py \
  --device cuda \
  --out-dir "$RUN_OUT" \
...
"$PYTHON_BIN" -m src.utils.research_os result-upsert \
  ...
  --config-path "scripts/mistral_soft_break_latebundle_v1.py --max-new-tokens $MAX_NEW_TOKENS --temperature $TEMPERATURE --top-p $TOP_P" \
  --prompt-contract mistral_soft_break_latebundle \
  --metric-path "late-stack anti-steer on recursive and baseline prompts + BT_ART + MALFORMED" \
  --claim-id "$CLAIM_ID"
