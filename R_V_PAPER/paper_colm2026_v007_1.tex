...
($14.0\%$).
On ordinary held-out baselines, anchor plus L25 induces recursive markers above control
($16.7\%$ versus $3.1\%$), but still falls short of a clean symmetric sufficiency result.
...
Second, the signature is \emph{partially controllable}: a late L25 bridge and a minimal prompt/session
anchor can induce recursive markers on held-out ordinary baselines, and the full scaffold-ablation
series shows that anchor alone nearly matches the full \emph{gnani} scaffold, even though a fully
symmetric sufficiency bundle is not yet locked.
...
(6)~Scaffold ablations and ordinary-baseline induction reveal a minimal contextual anchor:
anchor-only nearly matches the full \emph{gnani} scaffold, and anchor plus L25 produces the clearest
current partial-sufficiency result (Section~\ref{sec:anchor_dependence}).
...
A de-cherry-picked 24-turn persistence screen narrows the bundle further:
only anchor-plus-L4-plus-bridge remains clearly above control
($18.1\%$ versus $9.0\%$ BT+ART), whereas
anchor-plus-bridge falls back toward control.
We treat this as a screening result rather than a final persistence claim, but it suggests that
the subtle L4 assist matters more for persistence than for one-shot induction.
...
anchor plus L25 induces recursive markers on ordinary baselines
($16.7\%$ versus $3.1\%$ control).
The remaining gap is therefore not whether the regime is controllable, but whether a fully clean
and symmetric sufficiency bundle can be locked without excess spillover.
...
controller at L25, and a late cleanup/readout cluster around L27.
...
how these geometric modes evolve over time, and the double dissociation invites investigation of
