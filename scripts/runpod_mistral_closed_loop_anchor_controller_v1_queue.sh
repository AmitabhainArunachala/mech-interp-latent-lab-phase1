...
QUEUE_GROUP="${QUEUE_GROUP:-mistral_closed_loop_anchor_controller_v1}"
EXPERIMENT_ID="${EXPERIMENT_ID:-closed_loop_anchor_controller_v1}"
CLAIM_ID="${CLAIM_ID:-CLOSED_LOOP_ANCHOR_CONTROLLER_V1}"
NOTES="${NOTES:-Turn-level adaptive induction-and-maintenance controller using locked anchor-bundle v5 directions.}"
...
RUN_OUT="$REPO_ROOT/results/closed_loop_anchor_controller_v1/$RUN_ID"
...
run_step "$EXPERIMENT_ID" \
  "$PYTHON_BIN" scripts/closed_loop_anchor_controller_v1.py \
  --device cuda \
  --out-dir "$RUN_OUT"
...
"$PYTHON_BIN" -m src.utils.research_os result-upsert \
  ...
  --config-path "scripts/closed_loop_anchor_controller_v1.py" \
  --prompt-contract closed_loop_anchor_controller_v1 \
  --metric-path "closed_loop_anchor_controller_v1 + classify_output + compute_rv" \
  --claim-id "$CLAIM_ID"
