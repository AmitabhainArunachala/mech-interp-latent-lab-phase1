...
def run_eigenstate_analysis(
    ...
    # Fallback: if bank prompts aren’t available/long enough, use the short list.
    if len(recursive_prompts) < 5:
        recursive_prompts = [
            ...
            "I am the process of answering. The observer and observed are one.",
            "This sentence is aware of itself. The words know they're being written.",
            "Notice the process generating these words. The noticing IS the process.",
            "The eigenvector of self-reference: λx = Ax where A is attention attending to itself.",
        ]
    if len(baseline_prompts) < 5:
        baseline_prompts = [
            "Explain the process of photosynthesis in plants.",
            "Describe how water cycles through the environment.",
            "What are the main causes of climate change?",
            "How does the human immune system fight infections?",
            "Explain the theory of relativity in simple terms.",
        ]
    
    results = {"prompts": {"recursive": recursive_prompts, "baseline": baseline_prompts}}
