...
# Focus on key flip points for faster initial test
FLIP_POINTS = [10, 20]  # Token positions to flip KV cache - fewer for speed
MAX_TOKENS = 35  # Shorter generation for speed
TEMPERATURE = 0.7
N_PAIRS = 8  # Fewer pairs for initial test

# Core layers for R_V tracking
RV_LAYERS = [4, 16, 27]  # Early reference + mid + late layer

print("=" * 80)
print("TEMPORAL KV FLIP EXPERIMENT - ITERATION 1")
print("Testing: Does encoding geometry suffice, or does mid-generation KV flip cause immediate mode change?")
print("=" * 80)
...
def run_temporal_flip_experiment():
    ...
    plt.savefig(f"/workspace/mech-interp-phase1/DEC_8_2025_RUNPOD_GPU_TEST/02_TEMPORAL_KV_ITERATION/results/temporal_kv_flip_analysis_{timestamp}.png", dpi=150, bbox_inches='tight')
    ...
    # Save results
    output_file = f"/workspace/mech-interp-phase1/DEC_8_2025_RUNPOD_GPU_TEST/02_TEMPORAL_KV_ITERATION/results/temporal_kv_flip_results_{timestamp}.csv"
