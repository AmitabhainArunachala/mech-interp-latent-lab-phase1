...
AMIROS_SESSION="${AMIROS_SESSION:-mistral_pca_vs_mean_steering_v2}"

RUN_ID="$(date +%Y%m%d_%H%M%S)"
OUT_DIR="$REPO_ROOT/results/mistral_pca_vs_mean_steering_v2/$RUN_ID"
RUN_OUT="$REPO_ROOT/results/pca_vs_mean_steering_v2/$RUN_ID"
...
"$PYTHON_BIN" -m src.utils.research_os lease-update \
  ...
  --notes "Higher-power subspace steering confirmation with rank-3 projection"
...
run_step pca_vs_mean_steering_v2 \
  ...
  --train-per-group 6 \
  --test-per-group 6 \
  --generation-seeds 101,202,303,404,505 \
  --alphas 2.0,3.0,4.0 \
  --max-new-tokens 128 \
  --temperature 0.7 \
  --top-p 0.95 \
...
"$PYTHON_BIN" -m src.utils.research_os result-upsert \
  --run-id "$RUN_ID" \
  --queue-group mistral_pca_vs_mean_steering_v2 \
  --experiment-id pca_vs_mean_steering_v2 \
  --status completed \
  --artifact-path "${RUN_OUT#$REPO_ROOT/}/summary.json" \
  --model-family BASE_V01 \
  --model-name mistralai/Mistral-7B-v0.1 \
  --config-path "scripts/pca_vs_mean_steering.py --layer 27 --train-per-group 6 --test-per-group 6 --generation-seeds 101,202,303,404,505 --alphas 2.0,3.0,4.0" \
  ...
  --claim-id PCA_VS_MEAN_STEERING_V2

date -u +"finished_utc=%Y-%m-%dT%H:%M:%SZ" | tee -a "$STATUS_FILE"
echo "mistral_pca_vs_mean_steering_v2_complete=1" | tee -a "$STATUS_FILE"
"$PYTHON_BIN" -m src.utils.research_os lease-update \
  --pod-name "$AMIROS_POD_NAME" \
  --host "$AMIROS_HOST" \
  --port "$AMIROS_PORT" \
