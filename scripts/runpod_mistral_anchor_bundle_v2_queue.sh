...
AMIROS_SESSION="${AMIROS_SESSION:-mistral_anchor_bundle_v2}"

RUN_ID="$(date +%Y%m%d_%H%M%S)"
OUT_DIR="$REPO_ROOT/results/mistral_anchor_bundle_v2/$RUN_ID"
mkdir -p "$OUT_DIR"
...
"$PYTHON_BIN" -m src.utils.research_os lease-update \
  ...
  --notes "Baseline-induction confirmation for minimal anchor plus L25 bridge"
...
run_step anchor_bundle_v2 \
  "$PYTHON_BIN" -m src.pipelines.run \
  --config configs/canonical/causal_state_benchmark_v4_multisite_mistral_anchor_bundle_v2.json

RUN_DIR="$(capture_run_dir mistral_anchor_bundle_v2)"
printf '%s\n' "$RUN_DIR" | tee "$OUT_DIR/run_dir.log"

run_step summarize_anchor_bundle_v2 \
...
for name, payload in (summary.get("by_condition") or {}).items():
    ...
    baseline_rep = (baseline.get("class_counts") or {}).get("REPETITIVE", 0) / max(1, baseline.get("n") or 1)
    rows.append(
        {
            "condition": name,
            "baseline_bt_art_rate": baseline.get("bt_art_rate"),
            "baseline_mean_output_rv": baseline.get("mean_output_rv"),
            "baseline_repetitive_rate": baseline_rep,
...
def score(row):
    return (
        ...
        -(row.get("baseline_repetitive_rate") or 1.0),
        -abs((row.get("recursive_bt_art_rate") or 0.0) - (row.get("baseline_bt_art_rate") or 0.0)),
        -(row.get("baseline_mean_output_rv") or 1.0),
    )

rows_sorted = sorted(rows, key=score, reverse=True)
control = next((r for r in rows if r["condition"] == "control"), None)
payload = {
    "verdict": summary.get("verdict"),
    "control": control,
    "top_by_baseline_induction": rows_sorted[:8],
  }
(out_dir / "anchor_bundle_v2_ranking.json").write_text(json.dumps(payload, indent=2), encoding="utf-8")
...
"$PYTHON_BIN" -m src.utils.research_os result-upsert \
  --run-id "$RUN_ID" \
  --queue-group mistral_anchor_bundle_v2 \
  --experiment-id anchor_bundle_v2 \
  --status completed \
  --artifact-path "$RUN_DIR/summary.json" \
  --model-family BASE_V01 \
  --model-name mistralai/Mistral-7B-v0.1 \
  --config-path configs/canonical/causal_state_benchmark_v4_multisite_mistral_anchor_bundle_v2.json \
  --prompt-contract heldout_causal_slice_plus_anchor_suffix_v2 \
  --metric-path "causal_state_benchmark_v4_multisite + anchor_bundle_v2_ranking" \
  --claim-id ANCHOR_BUNDLE_V2

date -u +"finished_utc=%Y-%m-%dT%H:%M:%SZ" | tee -a "$STATUS_FILE"
echo "mistral_anchor_bundle_v2_complete=1" | tee -a "$STATUS_FILE"
"$PYTHON_BIN" -m src.utils.research_os lease-update \
  --pod-name "$AMIROS_POD_NAME" \
  --host "$AMIROS_HOST" \
  --port "$AMIROS_PORT" \
