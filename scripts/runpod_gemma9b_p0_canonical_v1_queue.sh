...
QUEUE_GROUP="gemma9b_p0_canonical_v1"
EXPERIMENT_ID="gemma9b_p0_canonical_v1"
CLAIM_ID="GEMMA9B_P0_CANONICAL_V1"
MODEL_NAME="google/gemma-2-9b"
P0_N=80
PATH_N=20
# Gemma-2-9B has 42 layers; early ~15% = L6, late ~84% = L35
PATH_LAYERS="0 3 6 9 12 15 18 21 24 27 30 33 35 38 41"
NOTES="Frozen-contract P0 canonical + full path patching on Gemma-2-9B base. Upgrades provisional Nov 2025 result to locked provenance."
...
"$PYTHON_BIN" -m src.utils.research_os result-upsert \
  ...
  --experiment-id gemma9b_p0_canonical --status completed \
...
run_step gemma9b_full_path_patching \
...
if [[ -n "$AFTER_LATEST" && "$AFTER_LATEST" != "$BEFORE_LATEST" ]]; then
  "$PYTHON_BIN" -m src.utils.research_os result-upsert \
    ...
    --experiment-id gemma9b_full_path_patching --status completed \
