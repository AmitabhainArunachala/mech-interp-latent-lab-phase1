...
AMIROS_SESSION="${AMIROS_SESSION:-mistral_scaffold_ablation_v1}"

RUN_ID="$(date +%Y%m%d_%H%M%S)"
OUT_DIR="$REPO_ROOT/results/mistral_scaffold_ablation_v1/$RUN_ID"
mkdir -p "$OUT_DIR"
...
"$PYTHON_BIN" -m src.utils.research_os lease-update \
  ...
  --queue-group mistral_scaffold_ablation_v1 \
  --run-id "$RUN_ID" \
  --status running \
  --current-step queue_boot \
  --out-dir "${OUT_DIR#$REPO_ROOT/}" \
  --notes "Scaffold ladder + sustained gnani persistence lane"
...
SCAFFOLD_OUT="results/self_feeding_scaffold_ablation_v1/$RUN_ID"
run_step self_feeding_scaffold_ladder_v1 \
  ...
  --n-sessions "${SELF_FEED_N_SESSIONS:-10}" \
  --seed-start "${SELF_FEED_SEED_START:-20260313}" \
  --output "$SCAFFOLD_OUT"
capture_latest_artifact "$SCAFFOLD_OUT/self_feeding_summary_*.json" scaffold_ablation_summary | tee "$OUT_DIR/scaffold_ablation_summary_artifact.log"
SCAFFOLD_ARTIFACT="$(cat "$OUT_DIR/scaffold_ablation_summary_artifact.txt")"
"$PYTHON_BIN" -m src.utils.research_os result-upsert \
  --run-id "$RUN_ID" \
  --queue-group mistral_scaffold_ablation_v1 \
  --experiment-id scaffold_ablation_ladder_v1 \
  --status completed \
  ...
  --claim-id SCAFFOLD_ABLATION_V1

GNANI_OUT="results/sustained_gnani_v3_bundle_v4/$RUN_ID"
run_step sustained_gnani_v3_v4 \
  "$PYTHON_BIN" scripts/sustained_gnani_v3.py \
  --model mistralai/Mistral-7B-v0.1 \
  --device cuda \
  --max-turns "${GNANI_MAX_TURNS:-50}" \
  --max-new-tokens "${GNANI_MAX_NEW_TOKENS:-180}" \
  --temperature "${GNANI_TEMPERATURE:-0.7}" \
  --rep-penalty "${GNANI_REP_PENALTY:-1.35}" \
  --n-recursive "${GNANI_N_RECURSIVE:-16}" \
  --n-baseline "${GNANI_N_BASELINE:-16}" \
  --seed-start "${GNANI_SEED_START:-20260313}" \
  --output "$GNANI_OUT"
capture_latest_artifact "$GNANI_OUT/comparison_summary.json" sustained_gnani_summary_v4 | tee "$OUT_DIR/sustained_gnani_summary_v4_artifact.log"
GNANI_ARTIFACT="$(cat "$OUT_DIR/sustained_gnani_summary_v4_artifact.txt")"
"$PYTHON_BIN" -m src.utils.research_os result-upsert \
  --run-id "$RUN_ID" \
  --queue-group mistral_scaffold_ablation_v1 \
  --experiment-id sustained_gnani_v3_v4 \
  --status completed \
  ...
  --claim-id PERSISTENCE_GNANI_V4

date -u +"finished_utc=%Y-%m-%dT%H:%M:%SZ" | tee -a "$STATUS_FILE"
echo "mistral_scaffold_ablation_v1_complete=1" | tee -a "$STATUS_FILE"
"$PYTHON_BIN" -m src.utils.research_os lease-update \
  --pod-name "$AMIROS_POD_NAME" \
  --host "$AMIROS_HOST" \
  --port "$AMIROS_PORT" \
