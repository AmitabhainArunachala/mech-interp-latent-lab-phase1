...
OUT_DIR="$REPO_ROOT/results/mistral_control_system_n100/$RUN_ID"
mkdir -p "$OUT_DIR"
...
echo "note=mode_atlas_v1 remains frozen at 20 prompts per mode and is not eligible for this n100 queue" | tee -a "$OUT_DIR/STATUS.txt"
...
echo "mistral_control_system_n100_complete=1" | tee -a "$OUT_DIR/STATUS.txt"
