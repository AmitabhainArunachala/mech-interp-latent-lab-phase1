...
AMIROS_SESSION="${AMIROS_SESSION:-mistral_pca_vs_mean_steering_v1}"

RUN_ID="$(date +%Y%m%d_%H%M%S)"
OUT_DIR="$REPO_ROOT/results/mistral_pca_vs_mean_steering_v1/$RUN_ID"
RUN_OUT="$REPO_ROOT/results/pca_vs_mean_steering_v1/$RUN_ID"
...
"$PYTHON_BIN" -m src.utils.research_os lease-update \
  ...
  --queue-group mistral_pca_vs_mean_steering_v1 \
  --run-id "$RUN_ID" \
  --status running \
  --current-step queue_boot \
  --out-dir "${OUT_DIR#$REPO_ROOT/}" \
  --notes "Compare mean-difference, PCA PC1, and PCA-subspace steering on held-out prompts"
...
run_step pca_vs_mean_steering_v1 \
  "$PYTHON_BIN" scripts/pca_vs_mean_steering.py \
...
"$PYTHON_BIN" -m src.utils.research_os result-upsert \
  --run-id "$RUN_ID" \
  --queue-group mistral_pca_vs_mean_steering_v1 \
  --experiment-id pca_vs_mean_steering_v1 \
  --status completed \
  --artifact-path "${RUN_OUT#$REPO_ROOT/}/summary.json" \
  --model-family BASE_V01 \
  --model-name mistralai/Mistral-7B-v0.1 \
  --config-path "scripts/pca_vs_mean_steering.py --layer 27 --train-per-group 8 --test-per-group 4 --generation-seeds 101,202,303 --alphas 1.0,2.0,3.0" \
  --prompt-contract heldout_causal_slice_even_split \
  --metric-path "classify_output + score_behavior_strict + compute_rv_with_components" \
  --claim-id PCA_VS_MEAN_STEERING_V1

date -u +"finished_utc=%Y-%m-%dT%H:%M:%SZ" | tee -a "$STATUS_FILE"
echo "mistral_pca_vs_mean_steering_v1_complete=1" | tee -a "$STATUS_FILE"
"$PYTHON_BIN" -m src.utils.research_os lease-update \
  --pod-name "$AMIROS_POD_NAME" \
  --host "$AMIROS_HOST" \
  --port "$AMIROS_PORT" \
