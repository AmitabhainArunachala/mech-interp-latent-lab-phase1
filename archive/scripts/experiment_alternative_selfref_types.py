...
HAS_ALTERNATIVE = True  # now sourced from prompts/bank.json via PromptLoader
...
def run_alternative_selfref_test():
    ...
    loader = PromptLoader()
    print(f\"Prompt bank version: {loader.version}\")
    experiential_prompts = loader.get_by_pillar(\"dose_response\", limit=N_PER_TYPE, seed=42)
    
    # Get alternative types
    godelian_prompts = loader.get_by_group(\"godelian\", limit=N_PER_TYPE, seed=42)
    surrender_prompts = loader.get_by_group(\"surrender\", limit=N_PER_TYPE, seed=42)
    # In-bank naming (from REUSABLE_PROMPT_BANK): akram_vignan
    akram_prompts = loader.get_by_group(\"akram_vignan\", limit=N_PER_TYPE, seed=42)
