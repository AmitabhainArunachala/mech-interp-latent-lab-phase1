#!/usr/bin/env python3
"""
Staged induction-to-maintenance handoff for the reduced Mistral protocol.

Goal:
- test whether a brief active handoff from the best static inducer
  (`anchor_drop_L25_vproj_bridge_3`) to the best reduced maintainer
  (`anchor_late_only_bridge_3`) extends persistence over a pure seed-only
  intervention
- keep the experiment tightly aligned with the current sufficiency story
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Staged induction-to-maintenance handoff")
    ...
    parser.add_argument("--experiment-name", default="staged_anchor_handoff_v1")
    ...
    out_dir = (
        Path(args.out_dir)
        if args.out_dir
        else REPO_ROOT / f"results/staged_anchor_handoff_v1/{timestamp}"
    )
        ...
        for generation_seed in args.generation_seeds:
            for condition_name in condition_names:
                ...
                for turn_idx in range(args.max_turns):
                    ...
                    # Empty generations can occur rarely; preserve the last non-empty
                    # context so the rollout remains well-posed on the next turn.
