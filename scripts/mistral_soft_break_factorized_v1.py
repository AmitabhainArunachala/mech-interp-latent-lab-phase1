#!/usr/bin/env python3
"""
Factorized late-stack anti-break test at a fixed sweep-selected scale.

Once the sweep finds the best shared scale, this script asks which component
or component pair is actually carrying the selective break.
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Factorized anti-late soft break test")
    ...
    parser.add_argument("--experiment-name", default="mistral_soft_break_factorized_v1")
    ...
    lowered = args.token_window.strip().lower()
    if lowered in {"full", "none", "all"}:
        token_window = None
    else:
        token_window = int(lowered)
    ...
    conditions = [
        {"name": "control", "hooks": []},
        {"name": "anti_l25_only", "hooks": [make_vproj_hook(25, "orthogonal_residual", BASE_L25 * scale)]},
        {"name": "anti_l27_only", "hooks": [make_vproj_hook(27, "subspace3_parallel", BASE_L27 * scale)]},
        {"name": "anti_bridge_only", "hooks": [make_bridge_hook(BASE_BRIDGE * scale)]},
        {
            "name": "anti_l25_l27",
            "hooks": [
    ...
    summary["verdict"] = {
        ...
        "factor_ranking": sorted(ranking, key=lambda row: row["selectivity_score"], reverse=True),
    }
