#!/usr/bin/env python3
"""
FULL VALIDATION of Layer 27 causal effect in Llama-3-8B-Instruct
REPLICATION of Mistral-7B protocol
Scales to n=45 with proper controls
Date: December 3, 2025

Exact replication of mistral_L27_FULL_VALIDATION.py methodology.
Only changes: Model name and output filenames.
"""
...
# PROVEN CONFIGURATION (identical to Mistral)
TARGET_LAYER = 27  # ✅ Confirmed critical layer (84% depth: 27/32)
EARLY_LAYER = 5    # ✅ Baseline layer (15.6% depth: 5/32)
...
def run_patched_forward(model, tokenizer, baseline_text, patch_source, 
    ...
    target_layer: Layer where patching occurs
    measure_layer: Layer where V is captured for R_V calculation (defaults to target_layer)
                    For wrong-layer control: patch at wrong_layer, measure at TARGET_LAYER
    """
    ...
    with torch.no_grad():
        ...
        def patch_at_target(m, i, o):
            """Hook at target_layer: patches the V values"""
            out = o.clone()
            B, T, D = out.shape
            ...
            if patch_type == "recursive":
                ...
                # No patch (baseline)
                if measure_layer == target_layer:
                    v_measure_list.append(out.detach())
                return out
            ...
            # If measuring at same layer, capture patched output
            if measure_layer == target_layer:
                v_measure_list.append(out.detach())
        ...
        def capture_measure(m, i, o):
            """Hook at measure_layer: captures V for R_V calculation"""
            v_measure_list.append(o.detach())
            return o
        
        # Register hooks
        h_early = model.model.layers[EARLY_LAYER].self_attn.v_proj.register_forward_hook(capture_early)
        h_target = model.model.layers[target_layer].self_attn.v_proj.register_forward_hook(patch_at_target)
        
        # If measuring at different layer than patching, register separate hook
        if measure_layer != target_layer:
    ...
    # Get measured V (from measure_layer, which may differ from target_layer)
    if v_measure_list:
        v_target = v_measure_list[0][0]
    else:
        v_target = None
...
def run_full_validation(model, tokenizer, prompt_bank, max_pairs=45):
    ...
    Identical to Mistral-7B protocol
    """
    print("="*70)
    print("LLAMA-3-8B LAYER 27 CAUSAL VALIDATION - FULL SCALE")
    print("="*70)
    ...
    for idx, (rec_id, base_id, rec_group, base_group) in enumerate(tqdm(pairs, desc="Processing")):
        ...
        try:
            ...
            # Patch at layer 21, but measure R_V at layer 27 (target layer)
            # This proves that patching at wrong layer doesn't affect target layer
            wrong_layer = 21
            ...
            v5_patch_wrong, v27_patch_wrong = run_patched_forward(
                model, tokenizer, base_text, v_wrong, "recursive", 
                target_layer=wrong_layer,  # Patch at layer 21
                measure_layer=TARGET_LAYER  # But measure at layer 27
            )
    ...
    filename = f"results/llama3_L27_FULL_VALIDATION_{timestamp}.csv"
    ...
    if gap != 0:
        ...
        elif abs(transfer) > 50:
            print("   ✅ STRONG TRANSFER - Universal mechanism confirmed!")
        elif abs(transfer) > 20:
            print("   ⚠️  PARTIAL TRANSFER - Investigate differences")
        else:
            print("   ❌ WEAK TRANSFER - Architecture-specific mechanism")
    ...
    try:
        ...
        plt.tight_layout()
        plot_filename = f'results/llama3_L27_validation_{timestamp}.png'
...
if __name__ == "__main__":
    ...
    print("from llama3_L27_FULL_VALIDATION import run_full_validation")
    print("from n300_mistral_test_prompt_bank import prompt_bank_1c")
