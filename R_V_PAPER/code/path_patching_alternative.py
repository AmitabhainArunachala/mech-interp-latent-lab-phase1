#!/usr/bin/env python3
"""
ALTERNATIVE PATH PATCHING: Using traditional PyTorch hooks
No nnsight required - builds on what already works
"""
...
def patch_residual_at_layer(model, layer_idx, patch_source):
    """
    Patch the residual stream (hidden states) at the input to a layer
    """
    handle = None
    ...
    try:
        # Register pre-hook on the target layer to patch its input
...
def get_residual_at_layer(model, tokenizer, text, layer_idx):
    """Get residual stream (hidden states) at output of a layer"""
    inputs = tokenizer(text, return_tensors="pt", truncation=True, max_length=512).to(model.device)
    ...
    with torch.no_grad():
        ...
        # Capture the output of the layer (which becomes input to next layer)
...
def run_path_patching_experiment(model, tokenizer, prompt_bank, n_pairs=30):
    """
    Path patching using traditional PyTorch hooks
    Patch residual at L27, measure V at downstream layers
    """
    print("="*70)
    print("PATH PATCHING: L27 RESIDUAL → DOWNSTREAM V MEASUREMENTS")
    print("="*70)
    print("Using traditional PyTorch hooks (no nnsight)")
    print("="*70)

    # Load pairs from your existing data
    ...
    for idx, row in tqdm(test_pairs.iterrows(), total=n_pairs, desc="Path patching"):
        ...
        try:
            ...
            with patch_residual_at_layer(model, TARGET_LAYER + 1, rec_residual):
                ...
                # Now measure V at all downstream layers
    ...
    path_df.to_csv('path_patching_L27-L31_pytorch.csv', index=False)

    print()
    print("="*70)
    print(f"PATH PATCHING: {len(path_df)} valid pairs")
    ...
    if errors and len(path_df) == 0:
        ...
        print("   4. Try single pair first: N_TEST_PAIRS=1")

    if len(path_df) > 5:
        print("\nR_V PROGRESSION (PATH PATCHING):")
        print("-"*70)
        ...
        if retention > 80:
            ...
            print("⚠️  STRONG REVERSAL")
        else:
            print("⚠️  PARTIAL ATTENUATION")

        print()
        print("✅ Saved: path_patching_L27-L31_pytorch.csv")

    print("="*70)

if __name__ == "__main__":
    print("Run in notebook:")
    print("from path_patching_alternative import run_path_patching_experiment")
    print("results = run_path_patching_experiment(model, tokenizer, prompt_bank_1c)")
