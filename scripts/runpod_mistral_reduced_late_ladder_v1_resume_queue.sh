...
QUEUE_GROUP="${QUEUE_GROUP:-mistral_reduced_late_ladder_v1_resume}"
PARENT_RUN_ID="${PARENT_RUN_ID:-20260320_125708}"
NOTES="${NOTES:-Resume reduced-late ladder after syncing low_rv-capable induced_persistence_followup.py.}"
...
RUN_OUT="$REPO_ROOT/results/mistral_reduced_late_ladder_v1_bundle/$PARENT_RUN_ID"
mkdir -p "$OUT_DIR" "$RUN_OUT"

STATUS_FILE="$OUT_DIR/STATUS.txt"
echo "run_id=$RUN_ID" | tee "$STATUS_FILE"
echo "parent_run_id=$PARENT_RUN_ID" | tee -a "$STATUS_FILE"
...
upsert_result() {
  ...
  "$PYTHON_BIN" -m src.utils.research_os result-upsert \
    --run-id "$PARENT_RUN_ID-$stage_id" \
    --queue-group "mistral_reduced_late_ladder_v1" \
