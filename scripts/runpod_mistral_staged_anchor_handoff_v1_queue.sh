...
OUTPUT_STEM="${OUTPUT_STEM:-staged_anchor_handoff_v1}"
QUEUE_GROUP="${QUEUE_GROUP:-mistral_staged_anchor_handoff_v1}"
EXPERIMENT_ID="${EXPERIMENT_ID:-staged_anchor_handoff_v1}"
CLAIM_ID="${CLAIM_ID:-STAGED_ANCHOR_HANDOFF_V1}"
NOTES="${NOTES:-Minimal staged induction-to-maintenance handoff using the reduced Mistral candidates.}"
...
run_step "$EXPERIMENT_ID" \
  "$PYTHON_BIN" scripts/staged_anchor_handoff_v1.py \
  --device cuda \
  --out-dir "$RUN_OUT" \
...
"$PYTHON_BIN" -m src.utils.research_os result-upsert \
  ...
  --config-path "scripts/staged_anchor_handoff_v1.py --prompts-per-group $PROMPTS_PER_GROUP --generation-seeds $GENERATION_SEEDS --max-turns $MAX_TURNS --max-new-tokens $MAX_NEW_TOKENS" \
  --prompt-contract staged_anchor_handoff \
  --metric-path "turn-level handoff rollout + classify_output + compute_rv" \
  --claim-id "$CLAIM_ID"
