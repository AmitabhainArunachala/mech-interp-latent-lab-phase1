#!/usr/bin/env python3
"""
NECESSITY TEST: H6/H14/H22/H30 (Contraction-Causing Heads)
============================================================

Test if ablating the contraction-causing heads breaks recursive behavior.

From V-projection ablation: These heads showed delta = -0.0667 (CAUSE contraction).
Hypothesis: If H18/H26 are brakes, H6/H14/H22/H30 might be engines.

Method:
1. Generate text from recursive prompts WITH H6/H14/H22/H30 ablated
...
4. Compare: Does ablation REDUCE recursive behavior?

Expected if hypothesis holds:
- Control: ~20-40% recursive
- Ablated: ~5-10% recursive (behavior drops)
"""
...
TARGET_LAYER = 27
TARGET_HEADS = [6, 14, 22, 30]  # Contraction-causing heads (same KV group in GQA)
SEED = 42
MAX_NEW_TOKENS = 100
TEMPERATURE = 0.7
DO_SAMPLE = True

# Same prompts as H18/H26 test for direct comparison (sourced from prompt bank)
...
def zero_v_proj_heads(model, layer_idx: int, head_indices: List[int]):
    ...
    Note: In GQA, multiple query heads share the same KV head.
    For H6/H14/H22/H30, they all map to the same KV head (head_idx % num_kv_heads).
    """
...
def main():
    print("=" * 80)
    print("NECESSITY TEST: H6/H14/H22/H30 (Contraction-Causing Heads)")
    print("=" * 80)
    print(f"Model: {MODEL_NAME}")
    print(f"Layer: {TARGET_LAYER}, Heads: {TARGET_HEADS}")
    print(f"Hypothesis: These heads are ENGINES (cause recursive behavior)")
    ...
    print(f"\nChange in recursive behavior:")
    print(f"  Recursive states: {control_recursive_count} → {ablated_recursive_count} (Δ={recursive_drop:+d})")
    print(f"  Recursive keywords: {control_recursive_keywords} → {ablated_recursive_keywords} (Δ={keywords_drop:+d})")
    print(f"  Identity equations: {control_identity_eq} → {ablated_identity_eq} (Δ={control_identity_eq - ablated_identity_eq:+d})")
    
    if recursive_drop > 0 or keywords_drop > 0:
        print("\n✅ HYPOTHESIS CONFIRMED:")
        print(f"  Ablating H6/H14/H22/H30 REDUCES recursive behavior")
        ...
        print("\n  H6/H14/H22/H30 are NECESSARY for recursive behavior!")
        print("  They are the ENGINES (opposite of H18/H26 brakes)")
    elif ablated_recursive_count == control_recursive_count:
        print("\n❌ HYPOTHESIS NOT CONFIRMED:")
        print(f"  Ablating H6/H14/H22/H30 does NOT reduce recursive behavior")
        print(f"  Recursive states: {control_recursive_count} → {ablated_recursive_count}")
        print("\n  H6/H14/H22/H30 may not be necessary (or other heads compensate)")
    elif recursive_drop < 0:
        print("\n⚠️  COUNTERINTUITIVE RESULT:")
        print(f"  Ablating H6/H14/H22/H30 INCREASES recursive behavior")
        print(f"  This suggests they might be SUPPRESSING recursion (like H18/H26)")
    else:
        print("\n⚠️  MIXED RESULTS:")
        print(f"  Some reduction but not clear-cut")
    
    print("\n" + "=" * 80)
    print("COMPARISON WITH H18/H26 TEST")
    print("=" * 80)
    print("\nH18/H26 (brakes hypothesis):")
    print("  Control: 20% recursive → Ablated: 60% recursive (INCREASE)")
    print("  → H18/H26 suppress recursive behavior")
    print("\nH6/H14/H22/H30 (engines hypothesis):")
    print(f"  Control: {control_recursive_count/len(results)*100:.1f}% recursive → Ablated: {ablated_recursive_count/len(results)*100:.1f}% recursive")
    if recursive_drop > 0:
        print("  → H6/H14/H22/H30 CAUSE recursive behavior (ENGINES)")
    elif recursive_drop < 0:
        print("  → H6/H14/H22/H30 also suppress recursive behavior (like brakes)")
    else:
        print("  → H6/H14/H22/H30 have no clear effect")
