...
OUTPUT_STEM="${OUTPUT_STEM:-subspace_component_steering}"
QUEUE_GROUP="${QUEUE_GROUP:-mistral_subspace_component_steering}"
EXPERIMENT_ID="${EXPERIMENT_ID:-subspace_component_steering}"
CLAIM_ID="${CLAIM_ID:-SUBSPACE_COMPONENT_STEERING}"
LAYER="${LAYER:-27}"
...
NOTES="${NOTES:-Compare in-subspace vs orthogonal steering in a learned recursive subspace}"
...
"$PYTHON_BIN" -m src.utils.research_os result-upsert \
  ...
  --config-path "scripts/subspace_component_steering.py --layer $LAYER --train-per-group $TRAIN_PER_GROUP --test-per-group $TEST_PER_GROUP --generation-seeds $GENERATION_SEEDS --alphas $ALPHAS" \
