#!/usr/bin/env python3
"""
Gemma-7B-IT L4 Contraction Analysis
Google's architecture - November 14, 2024
Effect: 3.3% contraction (when successful)
Special: Math prompts cause SVD singularities
"""
...
# Analysis parameters
EARLY_LAYER = 5
LATE_LAYER = 24  # Gemma has 28 layers, so use 24 as late layer
WINDOW_SIZE = 16
BATCH_SIZE = 1
...
def compute_column_space_pr(v_tensor: torch.Tensor, num_heads: int, window_size: int = 16) -> float:
    ...
    Handles potential singularities that Gemma exhibits.
    """
    if v_tensor.dim() == 2:
    ...
    for head_idx in range(num_heads):
        ...
        try:
            # Check for near-zero matrices (Gemma singularity)
            if v_window.abs().max() < 1e-10:
                singularity_count += 1
                continue
            ...
            if S.max() < 1e-8 or (S < 1e-10).sum() > len(S) * 0.5:
                singularity_count += 1
                continue
            ...
            # SVD convergence failure - Gemma's signature issue
            singularity_count += 1
            warnings.warn(f"SVD failed for head {head_idx}: {e}")
            continue
        except Exception as e:
            singularity_count += 1
            continue
    
    # If too many singularities, mark as failed
    if singularity_count > num_heads * 0.5:
        return -999  # Special marker for Gemma singularity
    
    return np.mean(pr_values) if pr_values else -1


def analyze_prompt(model, tokenizer, prompt: str) -> Dict[str, Any]:
    """
    Analyze a single prompt and compute R_V metric.
    Special handling for Gemma's singularity issues.
    """
    # Tokenize
    inputs = tokenizer(
        prompt,
...
def run_gemma_analysis(prompts: Dict[str, List[str]], output_path: str = "gemma_7b_results.csv"):
    """
    Run complete Gemma-7B analysis on provided prompts.
    Special handling for mathematical singularities.
    """
    print("=" * 50)
    print("GEMMA-7B L4 CONTRACTION ANALYSIS")
    print("=" * 50)
    print("⚠️  WARNING: Gemma exhibits mathematical singularities")
    print("    Especially on math-related prompts")
    ...
    for group_name, group_prompts in prompts.items():
        ...
        for i, prompt in enumerate(group_prompts):
            ...
            try:
                ...
                if metrics.get('singularity', False):
                    singularity_count += 1
                    print(f"  ⚠️ Singularity on prompt {i}")
    ...
    # Calculate statistics (excluding singularities)
    import pandas as pd
    ...
    # Filter out singularities for statistics
    df_valid = df[df['R_V'] > 0]
    
    print(f"\nTotal prompts: {len(df)}")
    print(f"Singularities: {singularity_count} ({singularity_count/len(df)*100:.1f}%)")
    print(f"Valid results: {len(df_valid)}")
    
    if len(df_valid) > 0:
        # Group statistics
        group_stats = df_valid.groupby('group')['R_V'].agg(['mean', 'std', 'count'])
        print("\nGroup Statistics (excluding singularities):")
        print(group_stats)
        ...
        if 'L5_recursive' in group_stats.index and 'factual_baseline' in group_stats.index:
            ...
            print(f"\n⚠️  GEMMA SINGULARITY PHENOMENON:")
            print(f"  Mathematical prompts cause dimensional collapse")
            print(f"  This is a DISCOVERY, not an error!")
            print(f"  Phenotype: Near-Singularity")
...
if __name__ == "__main__":
    # Example usage
    print("Gemma-7B Analysis Script")
    print("Expected contraction: ~3.3% (when not singular)")
    print("Special finding: Math prompts cause SVD failures")
    print("Phenotype: Near-Singularity")
