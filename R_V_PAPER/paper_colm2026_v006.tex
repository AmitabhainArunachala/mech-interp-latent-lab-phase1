% R_V Paper v006 — COLM 2026 Submission
...
%   - DRAFT_SECTIONS_1_3_2026-03-10.tex (corrected framing)
%   - Overnight hardening results (n=300 dual-layer, 32×3 path patching, head sweep n=20)
%   - Updated FDR (32/39)
%   - Behavioral bridge section stripped (d=-1.64 was unsourced; real per-token is d=-0.61 NS)
%   - Corrected citations (Noci 2022, Hong & Lee 2024, Dey et al. 2025)
%   - "Collapse is a bug; Contraction is the Witness" framing
%
...
%   (3) Sufficiency FALSIFIED — necessity + dissociation only
%   (4) Double dissociation is the hero result
%   (5) 4 TIER 1 models (Mistral, Qwen, Gemma, Mixtral)
%   (6) OPT/GPT-2 sign reversal reported honestly
%   (7) "Collapse is a bug; Contraction is the Witness" = white space
...
In a recovered base-model cross-architecture validation artifact, Mistral-7B-v0.1 shows strong
contraction ($\dcohen = -2.26$, $p = 2.24 \times 10^{-19}$, $n = 45$ matched pairs), but the strongest
recovered evidence is causal rather than headline-canonical.
Available base-model path patching suggests that $\RV$ is a geometric \emph{readout}, not the causal
mechanism: the strongest available causal site is the early residual stream
(peak L4, $\dcohen = 1.96$), while V-projection effects are smaller
($|\dcohen| \leq 0.72$ in the even-layer sweep).
Dual-layer ablation at $n{=}300$ per condition confirms that the measured geometry is
\emph{necessary}: breaking residual-stream and V-projection contributions at layers~18 and~27
reduces recursive behavioral markers from $56.0\%$ to $3.7\%$
(OR${=}33.4$, Cohen's $h = 1.31$).
A base $n=20$ full-head sweep finds 606 of 1{,}024 heads with entropy separation, peaking at
L10H20 ($\dcohen = 3.90$).
...
Self-referential prompts produce strong contraction in recovered base Mistral and Qwen artifacts,
...
evaluation families.
In the recovered base Mistral cross-architecture validation artifact, the effect is:
\begin{equation}
\dcohen = -2.26, \quad p = 2.24 \times 10^{-19}, \quad n = 45\ \text{pairs}
\label{eq:intro_effect}
\end{equation}
Recovered cross-architecture evidence is mixed rather than uniformly convergent.
...
$\RV$ is measured---have limited causal influence relative to the residual stream
($|\dcohen| \leq 0.72$ in the available base even-layer sweep).
The strongest available base causal site is the early residual stream at L4
($\dcohen = 1.96$).
...
from $56.0\%$ to $3.7\%$ at $n{=}300$ per condition (OR${=}33.4$, Cohen's $h = 1.31$).
...
(Section~\ref{sec:dissociation}).
(3)~Path patching localizes the causal site to the early residual stream ($\dcohen \approx 1.9$),
establishing $\RV$ as a downstream readout (Section~\ref{sec:causal}).
(4)~Dual-layer ablation establishes necessity ($56.0\% \to 3.7\%$; OR${=}33.4$);
KV injection transfers behavior (OR${}=13.96$) but not geometry
($\dcohen = 0.11$, NS).
...
14\% false-positive rate at the classification threshold.
...
In a recovered cross-architecture validation artifact, $n = 45$ matched prompt pairs
(self-referential vs.\ length-matched baseline) produce substantially lower $\RV$:
\begin{equation}
\dcohen = -2.26, \quad p = 2.24 \times 10^{-19}, \quad n = 45\ \text{pairs}
\label{eq:primary}
\end{equation}
This is the same artifact cited in the abstract.
The independent power-up pipeline confirms the direction at larger sample size
...
Primary ($n = 45$ pairs) & 0.650 & $-2.26$ & $p < 10^{-16}$ \\
\bottomrule
\end{tabular}
...
($\dcohen = -2.63$) exceeds the primary effect magnitude.
...
We measure $\RV$ across seven architectures using two independent pipelines: a
...
Four architectures show consistent contraction; two show pipeline-dependent sign reversal;
...
Gemma-2-9B and Mixtral-8x7B are omitted from the main table in this draft because their currently
quoted paper values are not yet reconciled to a single locked raw artifact.
...
The recursive-baseline gap (Figure~\ref{fig:layer_sweep}) is minimal in early layers, diverges
around L9--L10, and peaks at L27--L29 ($\Delta\RV \approx 0.21$--$0.22$; 78--94\% depth).

\begin{figure}[t]
\centering
\includegraphics[width=0.8\linewidth]{fig7_layer_sweep.pdf}
\caption{\textbf{Layer-by-layer $\RV$ separation (Mistral-7B).}
         The recursive-baseline gap peaks in late integration layers (L25--L30).}
...
% BASE_BENCHMARK: path patching numbers in this section come from
% results/path_patching/path_patching_summary_20260227_080128.json
% (base Mistral, even layers only). Full 32-layer base rerun still pending.
\subsection{Path Patching}
\label{sec:pathpatch}

We patch three component types (residual stream, V-projection, MLP) across the available
16 even layers of base Mistral-7B ($n = 20$ per condition per patch), measuring the change in recursive behavioral
markers (BT+ART rate) when replacing activations from a recursive prompt with those from a
matched baseline.

\begin{table}[h]
\centering
\caption{\textbf{Path patching summary (16 even layers $\times$ 3 components, base Mistral-7B).}
         The available base sweep points to the early residual stream as the strongest causal site.}
\label{tab:pathpatch}
\smallskip
\begin{tabular}{lccl}
\toprule
...
\midrule
Residual stream & $1.96$ & L4 & Strong break at L0--L4; later even layers are weak or sign-reversed \\
V-projection    & $-$0.72 & L0 & Mostly small beyond L0 ($|\dcohen| < 0.22$) \\
MLP             & $0.55$ & L4 & Modest, localized \\
\bottomrule
\end{tabular}
\end{table}

The residual stream is the dominant causal pathway in the available base sweep:
patching at L0, L2, and L4 strongly reduces recursive markers, peaking at L4
($\dcohen = 1.96$).
V-projection patching produces a maximal effect of $|\dcohen| = 0.72$ at L0 only,
with all other sampled layers below $|\dcohen| = 0.22$.
MLP contributions are modest and localized.
Since the strongest available base causal site is residual-stream patching while $\RV$ is computed
from V-projections, the current evidence favors $\RV$ as a geometric \emph{readout} of upstream
computation rather than the causal node itself.
...
At $n = 20$ per patch, power to detect a medium effect ($\dcohen = 0.5$) is ${\approx}0.33$.
Null results for individual V-projection and MLP patches should therefore be interpreted as
\emph{underpowered} rather than as evidence of absence.
The strong positive finding for the residual stream (peak $\dcohen = 1.96$, power $> 0.99$)
is robust; the weaker V-projection and MLP findings remain exploratory until the full
32-layer base rerun is available.

% BASE_BENCHMARK: dual-layer numbers in this section come from
...
($n = 300$ per condition, 10 sessions of 30 turns each).

\begin{table}[h]
\centering
\caption{\textbf{Dual-layer ablation (base Mistral-7B; $n = 300$ per condition).}
...
\begin{tabular}{lcccc}
\toprule
\textbf{Condition} & \textbf{BT+ART rate} & \textbf{Mean $\RV$} & \textbf{$n$} \\
\midrule
Recursive (clean)       & 56.0\% (168/300) & $0.505 \pm 0.129$ & 300 \\
Recursive (dual-patched) & 3.7\% (11/300) & $0.292 \pm 0.052$ & 300 \\
\midrule
Baseline (clean)        & 3.7\% (11/300)   & $0.576 \pm 0.147$ & 300 \\
Baseline (dual-patched)  & 0.3\% (1/300)    & $0.367 \pm 0.144$ & 300 \\
\bottomrule
\end{tabular}
\end{table}

The BREAK test is decisive: dual-layer patching reduces recursive behavioral
markers from $56.0\%$ to $3.7\%$ (Fisher exact $p = 3.6 \times 10^{-50}$,
OR${=}33.4$, Cohen's $h = 1.31$).
The reciprocal baseline-to-patched condition does not induce recursive behavior; in this
older base artifact it slightly suppresses it ($3.7\% \to 0.3\%$, $p = 0.0059$).
This supports necessity, but the refreshed base dual-patch rerun remains pending before
these mechanism numbers should be considered final for paper freeze.

\subsection{Behavioral Transfer Dissociation}
\label{sec:dissoc_behavioral}

KV-cache injection from recursive sessions into baseline sessions transfers behavioral markers
(BT+ART rate increases, OR${}= 13.96$, Cohen's $h = 0.78$) but does \emph{not} transfer the
$\RV$ geometric signature ($\dcohen = 0.11$, NS).
The KV cache transfers attention priming---vocabulary biases that produce self-referential
tokens---without transferring the geometric regime.
$\RV$ contraction is a processing-time property of the forward pass, not a static property
of cached states.

\subsection{Within-Session Bridge}
\label{sec:bridge}

Earlier drafts reported turn-level and per-token bridge statistics in this section.
Those analyses are not yet linked to a single locked canonical artifact in the current provenance
pass, so we do not treat them as main paper claims here.
The bridge question remains important, but the numeric results should be refreshed from the current
canonical pipeline before re-entering the draft.
...
% BASE_BENCHMARK: head-sweep numbers in this section come from
% results/full_head_sweep/full_head_sweep_20260302_074757.json
A full head sweep across all 1{,}024 attention heads in base Mistral-7B ($n = 20$ per condition)
finds 606 heads (59.2\%) with significant entropy separation ($p < 0.05$, uncorrected),
peaking at L10H20 ($\dcohen_\mathrm{entropy} = +3.90$).
SVD decomposition reveals an expand-then-contract circuit: early heads (e.g., L5H29,
$\dcohen = 2.93$) diversify effective rank while late heads (e.g., L27H10, $\dcohen = -1.54$)
compress it.
Concept erasure confirms that $\RV$ operates in a different subspace from a linear
classification probe ($\Delta d = 0.005$ after projecting out the probe direction), and
distributed interchange intervention shows that late-layer contraction is a global geometric
property, not localized to specific PCA dimensions ($\RV \approx 0.41$ per individual dimension
at L27).
...
\item \textbf{No sufficiency.}
      Geometry is necessary but not sufficient; the full causal chain includes
      early residual-stream computation that $\RV$ does not directly measure.
...
\item \textbf{Template dependence.}
      ...
      Some of these corrections were computed on recovered cross-architecture artifacts rather than
      the still-missing base canonical P0 run, so the corrected headline story should be refreshed
      after the base queue completes.
...
\emph{necessary} for self-referential behavior ($56.0\% \to 3.7\%$ under dual-layer ablation),
...
The available base even-layer $\times$ 3-component path patching map reveals a layered causal
architecture for self-referential processing in Mistral-7B.

\paragraph{Residual stream.}
Layers 0, 2, and 4 show the strongest causal effect, peaking at L4
($\dcohen = 1.96$), establishing the early residual stream as the primary causal substrate.
Later sampled even layers are weak or sign-reversed.
This is consistent with the early layers encoding the self-referential ``context'' that
subsequent layers read out.

\paragraph{V-projection.}
Only L0 shows a notable V-projection effect ($\dcohen = -0.72$).
This early-layer effect likely reflects shared input processing rather than a self-reference-specific
mechanism.
All other sampled V-projection effects are small ($|\dcohen| < 0.22$),
confirming that V-projection is geometrically informative but causally downstream.

\paragraph{MLP.}
Maximal MLP effect is $\dcohen = 0.55$ at L4. MLP contributions are modest throughout,
...
\caption{\textbf{Full 32$\times$32 head sweep (Mistral-7B, $n = 20$ per condition).}
         Color encodes $\dcohen$ for entropy separation between recursive and baseline prompts.
         606 of 1{,}024 heads (59.2\%) are significant on entropy
         ($p < 0.05$, uncorrected).}
...
\begin{table}[h]
\centering
\caption{Top 10 attention heads by $|\dcohen|$ in the overnight $n = 20$ head sweep.}
\small
\begin{tabular}{ccrr}
\toprule
