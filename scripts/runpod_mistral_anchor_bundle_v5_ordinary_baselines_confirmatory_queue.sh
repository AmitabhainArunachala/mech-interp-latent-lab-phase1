...
AMIROS_SESSION="${AMIROS_SESSION:-mistral_anchor_bundle_v5_ordinary_baselines_confirmatory}"

RUN_ID="$(date +%Y%m%d_%H%M%S)"
OUT_DIR="$REPO_ROOT/results/mistral_anchor_bundle_v5_ordinary_baselines_confirmatory/$RUN_ID"
mkdir -p "$OUT_DIR"
...
"$PYTHON_BIN" -m src.utils.research_os lease-update \
  ...
  --queue-group mistral_anchor_bundle_v5_ordinary_baselines_confirmatory \
  --run-id "$RUN_ID" \
  --status running \
  --current-step queue_boot \
  --out-dir "${OUT_DIR#$REPO_ROOT/}" \
  --notes "Confirm whether minimal anchor plus bridge generalizes beyond matched controls to ordinary baselines"
...
run_step anchor_bundle_v5_ordinary_baselines_confirmatory \
  "$PYTHON_BIN" -m src.pipelines.run \
  --config configs/canonical/causal_state_benchmark_v4_multisite_mistral_anchor_bundle_v5_ordinary_baselines_confirmatory.json

RUN_DIR="$(capture_run_dir mistral_anchor_bundle_v5_ordinary_baselines_confirmatory)"
printf '%s\n' "$RUN_DIR" | tee "$OUT_DIR/run_dir.log"

run_step summarize_anchor_bundle_v5_ordinary_baselines_confirmatory \
...
"$PYTHON_BIN" -m src.utils.research_os result-upsert \
  --run-id "$RUN_ID" \
  --queue-group mistral_anchor_bundle_v5_ordinary_baselines_confirmatory \
  --experiment-id anchor_bundle_v5_ordinary_baselines_confirmatory \
  --status completed \
  --artifact-path "$RUN_DIR/summary.json" \
  --model-family BASE_V01 \
  --model-name mistralai/Mistral-7B-v0.1 \
  --config-path configs/canonical/causal_state_benchmark_v4_multisite_mistral_anchor_bundle_v5_ordinary_baselines_confirmatory.json \
  ...
  --claim-id ANCHOR_BUNDLE_V5_ORDINARY

date -u +"finished_utc=%Y-%m-%dT%H:%M:%SZ" | tee -a "$STATUS_FILE"
echo "mistral_anchor_bundle_v5_ordinary_baselines_confirmatory_complete=1" | tee -a "$STATUS_FILE"
"$PYTHON_BIN" -m src.utils.research_os lease-update \
  --pod-name "$AMIROS_POD_NAME" \
  --host "$AMIROS_HOST" \
  --port "$AMIROS_PORT" \
