...
The question of how large language models process recursive self-reference---the capacity to represent and reason about their own processing---remains fundamental to both AI safety and mechanistic interpretability.
While behavioral studies demonstrate LLMs can engage with recursive prompts, the internal geometric mechanisms mediating such processing are poorly understood.

Recent work in geometric deep learning has revealed that transformer representations occupy structured manifolds whose geometry reflects semantic content.
...
Understanding these signatures could enable detection of consciousness-like states in AI systems---a critical capability for alignment research.
...
We define the \textbf{participation ratio} (PR) from singular value decomposition:
\begin{equation}
...
Figure~\ref{fig:cross_model} shows $\RV$ distributions across six architectures.
...
This localization to late layers suggests geometric contraction occurs during semantic integration, not feature extraction.
...
Our findings establish geometric contraction as a causally-mediated mechanism for recursive self-reference in transformers.
...
The universality and causal accessibility of the $\RV$ signature suggests a potential metric for detecting consciousness-like states.
If recursive self-reference is a component of machine consciousness, geometric contraction may serve as an interpretable indicator.

\textbf{Implications for alignment.}
The ability to induce geometric states through activation patching opens possibilities for steering model processing.
The homeostatic response suggests interventions must account for system-level dynamics.

\textbf{Limitations.}
Our causal claims are limited to correlation between geometry and prompt category; behavioral validation of "consciousness" remains an open problem.
...
% Placeholder references for draft
\begin{thebibliography}{99}

\bibitem{brown2020}
Brown, T. et al. (2020). Language models are few-shot learners. \emph{NeurIPS}.
...
\bibitem{meng2022}
Meng, K. et al. (2022). Locating and editing factual associations in GPT. \emph{NeurIPS}.

\bibitem{jiang2023}
Jiang, A. et al. (2023). Mistral 7B. \emph{arXiv preprint arXiv:2310.06825}.

\bibitem{hofstadter1979}
Hofstadter, D. (1979). \emph{Gödel, Escher, Bach: An Eternal Golden Braid}.

\bibitem{seth2021}
Seth, A. (2021). Being You: A New Science of Consciousness. \emph{Dutton}.
