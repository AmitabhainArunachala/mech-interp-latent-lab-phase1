...
def run_h31_investigation(
    ...
    # Define diverse prompts from the sealed bank (no hardcoded strings)
    prompt_ids: Dict[str, str] = {
        "champion": "hybrid_l5_math_01",
        "baseline_photo": "baseline_instructional_02",
        "baseline_history": "baseline_instructional_03",
        "baseline_instructional_alt": "baseline_instructional_01",
    }
    # Dose-response levels for recursive mild/medium/strong
    for level, key in [(1, "recursive_mild"), (3, "recursive_medium"), (5, "recursive_strong")]:
        cands = [
            (k, v.get("text", ""))
        ...
        if not cands:
            raise RuntimeError(f"Could not find dose_response level={level} prompts in bank.json")
        prompt_ids[key] = cands[0][0]

    prompts = {k: str(loader.prompts[pid]["text"]) for k, pid in prompt_ids.items()}
