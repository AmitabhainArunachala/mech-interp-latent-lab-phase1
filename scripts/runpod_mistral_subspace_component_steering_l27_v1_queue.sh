...
AMIROS_SESSION="${AMIROS_SESSION:-mistral_subspace_component_steering_l27_v1}"

RUN_ID="$(date +%Y%m%d_%H%M%S)"
OUT_DIR="$REPO_ROOT/results/mistral_subspace_component_steering_l27_v1/$RUN_ID"
RUN_OUT="$REPO_ROOT/results/subspace_component_steering_l27_v1/$RUN_ID"
...
"$PYTHON_BIN" -m src.utils.research_os lease-update \
  ...
  --queue-group mistral_subspace_component_steering_l27_v1 \
  --run-id "$RUN_ID" \
  --status running \
  --current-step queue_boot \
  --out-dir "${OUT_DIR#$REPO_ROOT/}" \
  --notes "Compare in-subspace vs orthogonal steering at the late L27 readout cluster"
...
run_step subspace_component_steering_l27_v1 \
  "$PYTHON_BIN" scripts/subspace_component_steering.py \
...
"$PYTHON_BIN" -m src.utils.research_os result-upsert \
  --run-id "$RUN_ID" \
  --queue-group mistral_subspace_component_steering_l27_v1 \
  --experiment-id subspace_component_steering_l27_v1 \
  --status completed \
  --artifact-path "${RUN_OUT#$REPO_ROOT/}/summary.json" \
  --model-family BASE_V01 \
  --model-name mistralai/Mistral-7B-v0.1 \
  --config-path "scripts/subspace_component_steering.py --layer 27 --train-per-group 6 --test-per-group 6 --generation-seeds 101,202,303 --alphas 2.0,3.0,4.0" \
  --prompt-contract heldout_causal_slice_even_split_v2 \
  --metric-path "in_subspace_vs_orthogonal_steering + classify_output + compute_rv_with_components" \
  --claim-id SUBSPACE_COMPONENT_STEERING_L27_V1

date -u +"finished_utc=%Y-%m-%dT%H:%M:%SZ" | tee -a "$STATUS_FILE"
echo "mistral_subspace_component_steering_l27_v1_complete=1" | tee -a "$STATUS_FILE"
"$PYTHON_BIN" -m src.utils.research_os lease-update \
  --pod-name "$AMIROS_POD_NAME" \
  --host "$AMIROS_HOST" \
  --port "$AMIROS_PORT" \
