...
AMIROS_SESSION="${AMIROS_SESSION:-mistral_anchor_bundle_v4_generalization_controls}"

RUN_ID="$(date +%Y%m%d_%H%M%S)"
OUT_DIR="$REPO_ROOT/results/mistral_anchor_bundle_v4_generalization_controls/$RUN_ID"
mkdir -p "$OUT_DIR"
...
"$PYTHON_BIN" -m src.utils.research_os lease-update \
  ...
  --queue-group mistral_anchor_bundle_v4_generalization_controls \
  --run-id "$RUN_ID" \
  --status running \
  --current-step queue_boot \
  --out-dir "${OUT_DIR#$REPO_ROOT/}" \
  --notes "Generalization test of anchor induction across matched and ordinary baselines"
...
run_step anchor_bundle_v4_generalization_controls \
  "$PYTHON_BIN" -m src.pipelines.run \
  --config configs/canonical/causal_state_benchmark_v4_multisite_mistral_anchor_bundle_v4_generalization_controls.json

RUN_DIR="$(capture_run_dir mistral_anchor_bundle_v4_generalization_controls)"
printf '%s\n' "$RUN_DIR" | tee "$OUT_DIR/run_dir.log"

run_step summarize_anchor_bundle_v4_generalization_controls \
...
for group_name in sorted({key[0] for key in grouped}):
    ...
    rows.sort(key=lambda row: (row["bt_art_rate"], -row["repetitive_rate"], -row["mean_output_rv"]), reverse=True)
...
"$PYTHON_BIN" -m src.utils.research_os result-upsert \
  --run-id "$RUN_ID" \
  --queue-group mistral_anchor_bundle_v4_generalization_controls \
  --experiment-id anchor_bundle_v4_generalization_controls \
  --status completed \
  --artifact-path "$RUN_DIR/summary.json" \
  --model-family BASE_V01 \
  --model-name mistralai/Mistral-7B-v0.1 \
  --config-path configs/canonical/causal_state_benchmark_v4_multisite_mistral_anchor_bundle_v4_generalization_controls.json \
  --prompt-contract champions_plus_matched_and_general_baselines \
  --metric-path "causal_state_benchmark_v4_multisite + baseline_group_ranking" \
  --claim-id ANCHOR_BUNDLE_V4_GENERALIZATION

date -u +"finished_utc=%Y-%m-%dT%H:%M:%SZ" | tee -a "$STATUS_FILE"
echo "mistral_anchor_bundle_v4_generalization_controls_complete=1" | tee -a "$STATUS_FILE"
"$PYTHON_BIN" -m src.utils.research_os lease-update \
  --pod-name "$AMIROS_POD_NAME" \
  --host "$AMIROS_HOST" \
  --port "$AMIROS_PORT" \
