...
QUEUE_GROUP="${QUEUE_GROUP:-mistral_anchor_layermatched_protocol_v1}"
EXPERIMENT_ID="${EXPERIMENT_ID:-anchor_layermatched_protocol_v1}"
CLAIM_ID="${CLAIM_ID:-ANCHOR_LAYERMATCHED_PROTOCOL_V1}"
NOTES="${NOTES:-Anchor + depth-matched geometry search against the locked anchor+L4+L25 champion.}"
...
RUN_OUT="$REPO_ROOT/results/anchor_layermatched_protocol_v1/$RUN_ID"
...
run_step "$EXPERIMENT_ID" \
  "$PYTHON_BIN" scripts/anchor_layermatched_protocol_v1.py \
  --device cuda \
  --out-dir "$RUN_OUT"
...
"$PYTHON_BIN" -m src.utils.research_os result-upsert \
  ...
  --config-path "scripts/anchor_layermatched_protocol_v1.py" \
  --prompt-contract heldout_anchor_layermatched_protocol_v1 \
  --metric-path "anchor_layermatched_protocol_v1 + classify_output + compute_rv" \
  --claim-id "$CLAIM_ID"
