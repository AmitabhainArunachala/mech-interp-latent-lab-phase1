...
OUTPUT_STEM="${OUTPUT_STEM:-mistral_soft_break_latebundle_sweep_v1}"
QUEUE_GROUP="${QUEUE_GROUP:-mistral_soft_break_latebundle_sweep_v1}"
EXPERIMENT_ID="${EXPERIMENT_ID:-mistral_soft_break_latebundle_sweep_v1}"
CLAIM_ID="${CLAIM_ID:-MISTRAL_SOFT_BREAK_LATEBUNDLE_SWEEP_V1}"
NOTES="${NOTES:-Dose sweep around the winning soft anti-late-only break setting.}"
SCALES="${SCALES:-0.25,0.5,0.75,1.0,1.25,1.5}"
...
run_step "$EXPERIMENT_ID" \
  "$PYTHON_BIN" scripts/mistral_soft_break_latebundle_sweep_v1.py \
  --device cuda \
  --out-dir "$RUN_OUT" \
...
"$PYTHON_BIN" -m src.utils.research_os result-upsert \
  ...
  --config-path "scripts/mistral_soft_break_latebundle_sweep_v1.py --scales $SCALES --max-new-tokens $MAX_NEW_TOKENS --temperature $TEMPERATURE --top-p $TOP_P" \
  --prompt-contract mistral_soft_break_latebundle_sweep \
  --metric-path "late-stack anti-steer sweep on recursive and baseline prompts + BT_ART + MALFORMED" \
  --claim-id "$CLAIM_ID"
