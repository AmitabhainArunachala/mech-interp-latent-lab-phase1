...
def run_l27_head_analysis(
    ...
    # Load prompts from the sealed prompt bank (DEC15 hygiene: no hardcoded strings)
    loader = PromptLoader()
    ...
    try:
        ...
        raise KeyError(
            f"Missing required prompt_id in prompts/bank.json: {e}. "
            f"Expected ids: {champion_id}, {baseline_id}"
        )
