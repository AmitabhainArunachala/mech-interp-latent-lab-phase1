...
def run_rv_l27_causal_validation_from_config(cfg: Dict[str, Any], run_dir: Path) -> ExperimentResult:
    ...
    if stats is not None:
        ...
        if arr.size > 1:
            ...
            tests["main_effect_ttest_1samp_less_0"] = {"t": float(t_stat), "p": float(p_val), "n": int(arr.size)}
        ...
        def _paired(a: List[float], b: List[float]) -> Dict[str, float]:
            ...
            if int(m.sum()) < 2:
                return {"t": float("nan"), "p": float("nan"), "n": float(m.sum())}
            t, p = stats.ttest_rel(aa[m], bb[m])
            return {"t": float(t), "p": float(p), "n": float(m.sum())}
    ...
    gap = float(np.nanmean(rv_base_list) - np.nanmean(rv_rec_list)) if rows else float("nan")
    transfer = float(np.nanmean(deltas_main) / gap * 100.0) if rows and gap not in (0.0, float("nan")) else float("nan")
