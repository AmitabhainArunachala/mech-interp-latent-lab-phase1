#!/usr/bin/env python3
"""Run L0+L1+L3 combined sufficiency test."""

import sys
from pathlib import Path
import json
from datetime import datetime

sys.path.insert(0, str(Path(__file__).parent.parent))

from src.pipelines.mlp_combined_sufficiency_test import run_combined_mlp_sufficiency_test_from_config

if __name__ == "__main__":
    cfg_path = Path(__file__).parent.parent / "configs" / "combined_mlp_sufficiency_l0_l1_l3.json"
    cfg = json.load(open(cfg_path))
    
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    run_dir = Path("results/phase1_mechanism/runs") / f"{timestamp}_l0_l1_l3_combined_sufficiency"
    run_dir.mkdir(parents=True, exist_ok=True)
    
    print(f"Run directory: {run_dir}")
    print(f"Layers: {cfg['params']['layers']}")
    print(f"n_pairs: {cfg['params']['n_pairs']}")
    print("")
    print("Starting experiment...")
    print("="*60)
    
    result = run_combined_mlp_sufficiency_test_from_config(cfg, run_dir)
    
    print("")
    print("="*60)
    print("✅ Experiment complete!")
    print(f"Results: {run_dir}")
