...
LOGDIR=results/rv_masterplan/phase3_${TIMESTAMP}
mkdir -p ${LOGDIR}

echo "=== Phase 3 Relaunch: $(date) ==="
echo "Log dir: ${LOGDIR}"
...
python scripts/sae_feature_analysis.py --model google/gemma-2-2b --device cuda 2>&1 | tee ${LOGDIR}/e3.1_sae.log
E31_EXIT=$?
echo "[E3.1] Exit code: ${E31_EXIT} — $(date)"

echo ""
echo "[E3.2] Circuit Tracing — starting $(date)"
python scripts/circuit_tracing_analysis.py --model google/gemma-2-2b --device cuda 2>&1 | tee ${LOGDIR}/e3.2_circuit.log
E32_EXIT=$?
echo "[E3.2] Exit code: ${E32_EXIT} — $(date)"

echo ""
echo "[E2.4] DII Intervention — starting $(date)"
python scripts/dii_intervention.py --model google/gemma-2-2b --device cuda 2>&1 | tee ${LOGDIR}/e2.4_dii.log
E24_EXIT=$?
