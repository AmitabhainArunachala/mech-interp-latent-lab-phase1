...
AMIROS_SESSION="${AMIROS_SESSION:-mistral_pca_subspace_ablation_l5_v1}"

RUN_ID="$(date +%Y%m%d_%H%M%S)"
OUT_DIR="$REPO_ROOT/results/mistral_pca_subspace_ablation_l5_v1/$RUN_ID"
RUN_OUT="$REPO_ROOT/results/pca_subspace_ablation_l5_v1/$RUN_ID"
...
"$PYTHON_BIN" -m src.utils.research_os lease-update \
  ...
  --queue-group mistral_pca_subspace_ablation_l5_v1 \
  --run-id "$RUN_ID" \
  --status running \
  --current-step queue_boot \
  --out-dir "${OUT_DIR#$REPO_ROOT/}" \
  --notes "Early-layer source test for projection-ablation to compare source-like vs late cleanup subspace behavior"
...
run_step pca_subspace_ablation_l5_v1 \
  "$PYTHON_BIN" scripts/pca_subspace_ablation.py \
...
"$PYTHON_BIN" -m src.utils.research_os result-upsert \
  --run-id "$RUN_ID" \
  --queue-group mistral_pca_subspace_ablation_l5_v1 \
  --experiment-id pca_subspace_ablation_l5_v1 \
  --status completed \
  --artifact-path "${RUN_OUT#$REPO_ROOT/}/summary.json" \
  --model-family BASE_V01 \
  --model-name mistralai/Mistral-7B-v0.1 \
  --config-path "scripts/pca_subspace_ablation.py --layer 5 --train-per-group 6 --test-per-group 6 --generation-seeds 101,202,303 --alphas 0.5,1.0" \
  --prompt-contract heldout_causal_slice_even_split_v2 \
  --metric-path "projection_ablation + classify_output + score_behavior_strict + compute_rv_with_components" \
  --claim-id PCA_SUBSPACE_ABLATION_L5_V1

date -u +"finished_utc=%Y-%m-%dT%H:%M:%SZ" | tee -a "$STATUS_FILE"
echo "mistral_pca_subspace_ablation_l5_v1_complete=1" | tee -a "$STATUS_FILE"
"$PYTHON_BIN" -m src.utils.research_os lease-update \
  --pod-name "$AMIROS_POD_NAME" \
  --host "$AMIROS_HOST" \
  --port "$AMIROS_PORT" \
