...
def run_phase1_existence_from_config(cfg, run_dir):
    ...
    # Battery
    categories = {
        "recursive": get_prompts_by_pillar(pillar="dose_response", limit=recursive_limit, seed=seed),
        "baseline": get_prompts_by_pillar(pillar="baselines", limit=baseline_limit, seed=seed),
    }
    rows = []
    for name, prompts in categories.items():
        for p in prompts:
