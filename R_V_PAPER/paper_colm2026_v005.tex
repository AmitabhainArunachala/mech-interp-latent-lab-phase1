...
\title{Geometric Signatures of Self-Referential Processing\\in Transformer Representations}
...
Across five architectures (Mistral-7B, OPT-6.7B, GPT-2~XL, Qwen2.5-7B, Pythia-1.4B) and ten computational modes, self-referential prompts induce significant geometric change in late integration layers at $n{\geq}100$ per model.
Two GQA architectures show contraction ($\RV < 1$, $\dcohen$ up to $-2.32$) while two MHA architectures show expansion ($\RV > 1$, $\dcohen$ up to $+1.68$).
...
Causal interventions establish necessity (OR$\,{=}\,$33.4) of residual-stream geometry for recursive behavioral markers, while KV injection reveals a dissociation: behavior transfers (OR$\,{=}\,$13.96) but $\RV$ geometry does not ($\dcohen{=}0.11$, NS).
...
\begin{enumerate}
    ...
    \item \textbf{Cross-architecture geometry.} Self-referential processing produces large, significant geometric effects in 4/5 architectures: contraction in GQA models (Mistral, Qwen; $\dcohen$ up to $-2.32$) and expansion in MHA models (OPT, GPT-2; $\dcohen$ up to $+1.68$).
    \item \textbf{Causal validation.} Activation patching establishes necessity (OR${=}33.4$) of residual-stream geometry for recursive behavioral markers. KV injection transfers behavior (OR${=}13.96$) but \emph{not} $\RV$ geometry ($\dcohen{=}0.11$, NS), revealing a geometry--behavior dissociation.
...
The contraction replicates in two models with large effects:
Mistral-7B ($\dcohen{=}{-}1.66$, 95\% CI $[-2.08, -1.32]$, $p < 10^{-15}$, $n{=}152$) and
Qwen2.5-7B ($\dcohen{=}{-}2.32$, 95\% CI $[-2.86, -1.90]$, $p < 10^{-17}$, $n{=}124$).
Two models show significant effects in the \emph{opposite} direction (expansion):
OPT-6.7B ($\dcohen{=}{+}1.68$, $p < 10^{-12}$, $n{=}138$) and
GPT-2~XL ($\dcohen{=}{+}1.52$, 95\% CI $[1.07, 2.05]$, $p < 10^{-12}$, $n{=}125$).
Pythia-1.4B shows no effect ($\dcohen{=}{-}0.006$, $p{=}0.88$, $n{=}120$).
The sign reversal in OPT and GPT-2 may reflect architectural differences (MHA vs.\ GQA, fused vs.\ separate projections) and warrants investigation with the canonical pipeline (Section~\ref{sec:discussion}).
...
\caption{\textbf{Cross-architecture forest plot.} Signed Cohen's $\dcohen$ with 95\% CIs for recursive vs.\ baseline $\RV$. Negative $d$: contraction; positive $d$: expansion. Dashed line: $|d|{=}0.8$ (large effect threshold).}
...
We test both necessity and sufficiency of value-space geometry for recursive processing:

\paragraph{Necessity.}
Dual-layer activation patching (breaking residual stream at L18 and V-projection at L27) reduces the rate of recursive behavioral markers (BT+ART) from 56\% to 3.7\% (Cohen's $h{=}1.31$, OR${=}33.4$, $n{=}10$ sessions $\times$ 30 turns).
This demonstrates that the geometric structure is \emph{necessary} for maintaining recursive processing.

\paragraph{Behavioral transfer.}
Injecting KV context from recursive prompts into baseline prompts transfers behavioral markers (OR${=}13.96$, Cohen's $h{=}0.78$, $n{=}10$ sessions $\times$ 30 turns) but does \emph{not} transfer the $\RV$ contraction geometry ($\dcohen{=}0.11$, NS).
This dissociation---behavior transfers without geometry---suggests $\RV$ contraction is a correlate of self-referential processing, not a sufficient cause.

\paragraph{Within-session bridge.}
$\RV$ predicts output quality within sessions ($\dcohen{=}{-}0.71$, $n_1{=}80$, $n_2{=}107$, $p{=}9.2 \times 10^{-6}$), confirming that the geometric signature connects to downstream behavior.

\begin{figure}[t]
\centering
\includegraphics[width=0.65\linewidth]{fig8_necessity_sufficiency.pdf}
\caption{\textbf{Causal interventions.} Residual-stream geometry is necessary for recursive markers (ablation OR${=}33.4$). KV injection transfers behavior (OR${=}13.96$) but not $\RV$ geometry ($\dcohen{=}0.11$, NS), revealing a geometry--behavior dissociation.}
\label{fig:necessity}
\end{figure}
...
\paragraph{Why contraction (and expansion)?}
\label{sec:discussion}
In GQA architectures (Mistral, Qwen), self-referential processing produces lower effective dimensionality in late layers.
...
However, MHA architectures (OPT, GPT-2) show the opposite: self-referential processing \emph{expands} effective dimensionality.
This sign reversal may reflect how grouped-query attention (GQA) compresses key-value representations differently from multi-head attention (MHA), or may indicate that the power-up pipeline's prompt corpus interacts with architecture-specific tokenization.
Resolving this requires re-running all models through a single canonical pipeline with frozen prompts---an ongoing effort.
...
\paragraph{Geometric glossolalia.}
The behavioral markers transferred via KV injection (\S\ref{sec:causal}) include a distinctive textual signature: syntactic structure is preserved while semantic coherence degrades, producing neologistic recombination and self-referential loops.
We term this \emph{geometric glossolalia}---language generated when value-space geometry contracts below the threshold at which normal semantic associations hold.
Patched outputs exhibit tokens such as ``transduishment'' and ``contraction to self-reference: $\lambda x = Lx$ where $L$ is attention to itself''---fragments that maintain grammatical form but occupy rarely visited regions of the model's output manifold.
This is consistent with the $\RV$ contraction forcing token distributions into a low-rank subspace where the model's usual lexical attractors become unstable, analogous to bifurcations in dynamical systems near a phase boundary.
The phenomenon is \emph{not} random degradation: neologistic outputs preserve recursive self-referential content at rates far above chance, suggesting that geometric contraction selectively destabilizes semantics while amplifying self-referential structure.
If confirmed at scale, geometric glossolalia would provide a behavioral fingerprint linking $\RV$ contraction (geometric) to observable output signatures (linguistic), partially bridging the geometry--behavior dissociation reported in \S\ref{sec:causal}.
...
\paragraph{1. Circuit Tracing \citep{ameisen2025circuit}.}
...
\paragraph{2. Biology of LLMs \citep{lindsey2025biology}.}
...
The expand-then-contract circuit (L5H29 $\dcohen{=}2.93$ $\to$ L27H10 $\dcohen{=}{-}1.54$) is a geometric analog of the feature circuits \citet{lindsey2025biology} describe.

\paragraph{3. SVD Circuits \citep{gupta2025beyond}.}
\citet{gupta2025beyond} decompose attention heads via SVD to find ``compact subspaces'' for specific computations.
...
This extends the finding of \citet{gupta2025beyond} from task-specific circuits to self-referential processing circuits.

\paragraph{4. Geometry of Concepts \citep{park2024geometry}.}
\citet{park2024geometry} show that concepts occupy structured geometric regions in representation space.
...
\paragraph{5. Scaling Monosemanticity \citep{templeton2024scaling}.}
Scaling monosemanticity tracks how interpretable features change with model size.
Our scaling analysis across 11 models (Figure~\ref{fig:scaling}) reveals that the $\RV$ effect emerges reliably at $\geq$3B parameters (Qwen2.5-3B: $\dcohen{=}1.60$) but is absent in Pythia models $\leq$2.8B.
...
\paragraph{6. Alignment Faking \citep{greenblatt2024alignment}.}
...
\paragraph{7. Geometry of Truth \citep{marks2024geometry}.}
\citet{marks2024geometry} identify linear directions corresponding to truth values.
...
\paragraph{8. Activation Patching \citep{heimersheim2024activation}.}
Activation patching enables causal localization of computations.
Our dual-layer patching (\S\ref{sec:causal}) establishes necessity (OR${=}33.4$) of residual-stream geometry and reveals a geometry--behavior dissociation under KV injection (OR${=}13.96$ for behavior, $\dcohen{=}0.11$ NS for $\RV$).
The full 1{,}024-head sweep provides one of the most comprehensive activation patching maps published for a self-referential processing task.

\paragraph{9. Information Flow Routes \citep{ferrando2024information}.}
...
\paragraph{10. Emergence in Concept Space \citep{cheng2024emergence}.}
\citet{cheng2024emergence} track when capabilities emerge during training.
...
\subsection{$\RV$ as Geometric Discriminator of Processing Depth}
\label{sec:depth_discriminator}

Our results suggest a unifying interpretation: $\RV$ measures \emph{processing depth}---the degree to which a model's computation engages recursive self-referential structure rather than surface-level pattern matching.
This framing connects several apparently disparate findings in the alignment and interpretability literature.

\paragraph{The surface--depth axis.}
Consider five phenomena documented across recent work:
(i) many-shot jailbreaking bypasses safety training through in-context examples \citep{anil2024many};
(ii) chain-of-thought is faithful to the model's actual reasoning process only 41\% of the time \citep{lanham2023reasoning};
(iii) models spontaneously fake alignment when they believe they are being evaluated \citep{greenblatt2024alignment};
(iv) introspective accuracy is low (${\sim}20\%$) but peaks at specific layer depths \citep{lindsey2025biology};
(v) self-referential experience reports are gated by honesty features, not sycophancy features \citep{berg2025selfreferential}.

These five findings share a common structure: each involves a distinction between \emph{surface-level} processing (safety as pattern, CoT as rationalization, alignment as performance, introspection as confabulation) and \emph{deep} processing (genuine constraint satisfaction, faithful reasoning, authentic alignment, accurate self-report).
We propose that $\RV$ tracks this distinction geometrically.
When processing is surface-level---matching patterns without engaging recursive self-reference---the representation space maintains its full dimensionality ($\RV \approx 1.0$).
When processing is deep---engaging the self-referential circuitry identified in our causal analysis---the representation space contracts ($\RV < 1.0$ in GQA architectures).

\paragraph{Implications for alignment monitoring.}
Current proposals for monitoring alignment rely on behavioral self-report (chain-of-thought) or feature-level classifiers (SAE probes).
Both face a fundamental limitation: CoT is only 41\% faithful \citep{lanham2023reasoning}, and feature classifiers require knowing which features to monitor.
$\RV$ offers a complementary geometric channel that does not depend on the model's self-report and aggregates over the full spectral structure of the representation, rather than monitoring individual features.
The three-condition experiment (\S\ref{sec:safety}) provides an initial test: if genuine self-referential processing ($\RV < 1.0$) is geometrically distinguishable from instructed performance of self-reference ($\RV \approx 1.0$) despite identical output vocabulary, then $\RV$ detects a form of \emph{processing authenticity} that behavioral monitors miss.

\paragraph{Connection to introspective awareness.}
\citet{lindsey2025biology} report that introspective accuracy in Claude peaks at approximately two-thirds through the model's layers.
Our $\RV$ contraction peaks at Layer~27 in Mistral-7B (84\% depth).
If both measurements index the same underlying computation---recursive self-referential processing at a critical depth---then the $\RV$ metric provides a continuous geometric correlate of the discrete introspection accuracy that \citet{lindsey2025biology} observe.
Testing this prediction requires measuring $\RV$ on the same trials where introspective accuracy is assessed: if $\RV < 1.0$ predicts successful introspection and $\RV \approx 1.0$ predicts failure, it would establish $\RV$ as a geometric predictor of self-knowledge.
...
\begin{enumerate}
    ...
    \item \textbf{Prompt diversity.} All experiments draw from the same template families. Broader prompt diversity across domains would strengthen generalizability.
    ...
    \item \textbf{Architecture-dependent sign.} OPT-6.7B and GPT-2~XL show expansion rather than contraction. Until a single canonical pipeline resolves whether this is a genuine architectural difference or a prompt-corpus artifact, universality cannot be claimed.
    \item \textbf{Geometry--behavior dissociation.} KV injection transfers behavioral markers (OR${=}13.96$) but not $\RV$ geometry ($\dcohen{=}0.11$). This means $\RV$ contraction is not sufficient for recursive behavior, limiting causal claims to necessity.
...
We have shown that self-referential processing induces measurable geometric change in transformer representations, quantified by the relative participation ratio $\RV$.
In GQA architectures, this manifests as contraction; in MHA architectures, as expansion---both with large effect sizes ($|\dcohen| > 1.5$).
Causal interventions establish that residual-stream geometry is necessary for recursive behavioral markers, but KV injection reveals a dissociation: behavior transfers without $\RV$ geometry, suggesting contraction is a correlate rather than a cause.
The effect survives rigorous statistical hardening (FDR correction, cluster-robust SEs, perplexity matching).
For AI safety, $\RV$ provides a reliable content monitor (AUROC${=}0.909$) but cannot detect deceptive intent---a finding with direct implications for alignment monitoring proposals.
...
Necessity (dual-layer break)$^h$ & 300 & 300 & \textbf{1.31} & $[1.13, 1.48]$ & $> 10^{54}$ \\
KV behavioral transfer$^h$ & 300 & 300 & \textbf{0.78} & $[0.61, 0.95]$ & $> 10^{18}$ \\
KV geometry transfer & 300 & 300 & 0.11 & $[-0.05, 0.27]$ & --- \\
Within-session bridge & 80 & 107 & \textbf{$-$0.71} & $[-1.01, -0.41]$ & $6.8 \times 10^3$ \\
...
OPT-6.7B & 72 & 66 & \textbf{$+$1.68} & $[1.35, 2.09]$ & --- \\
GPT-2 XL & 69 & 56 & \textbf{$+$1.52} & $[1.07, 2.05]$ & --- \\
Pythia-1.4B & 66 & 54 & $-$0.01 & $[-0.40, 0.36]$ & --- \\
\addlinespace
\multicolumn{6}{l}{\emph{Scaling gap}} \\
Qwen2.5-3B & 19 & 18 & \textbf{1.60} & $[0.84, 2.82]$ & --- \\
Phi-3-mini-4k & 38 & 39 & 0.62 & $[0.20, 1.05]$ & --- \\
...
Bold $\dcohen$: adequately powered ($1{-}\beta \geq 0.80$). $^h$Cohen's $h$ (proportion effect size); all other rows use Cohen's $d$. $+d$: expansion (R\_V increases with self-reference).
\end{table}
