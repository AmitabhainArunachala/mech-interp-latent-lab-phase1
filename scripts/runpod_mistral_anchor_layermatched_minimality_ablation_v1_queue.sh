...
OUTPUT_STEM="${OUTPUT_STEM:-anchor_layermatched_minimality_ablation_v1}"
QUEUE_GROUP="${QUEUE_GROUP:-mistral_anchor_layermatched_minimality_ablation_v1}"
EXPERIMENT_ID="${EXPERIMENT_ID:-anchor_layermatched_minimality_ablation_v1}"
CLAIM_ID="${CLAIM_ID:-ANCHOR_LAYERMATCHED_MINIMALITY_ABLATION_V1}"
NOTES="${NOTES:-Static minimality ablation for the confirmed layer-matched maintenance bundle.}"
...
run_step "$EXPERIMENT_ID" \
  "$PYTHON_BIN" scripts/anchor_layermatched_minimality_ablation_v1.py \
  --out-dir "$RUN_OUT"
...
"$PYTHON_BIN" -m src.utils.research_os result-upsert \
  ...
  --config-path "scripts/anchor_layermatched_minimality_ablation_v1.py" \
  --prompt-contract anchor_layermatched_minimality_static_ablation \
  --metric-path "static induction benchmark + classify_output + compute_rv" \
  --claim-id "$CLAIM_ID"
