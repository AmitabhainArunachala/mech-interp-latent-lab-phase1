...
Exact experimental conditions from mistral_L27_FULL_VALIDATION.py
"""
...
# ==============================================================================
# CONFIGURATION (NOV 16 EXACT REPRODUCTION)
# ==============================================================================
CONFIG = {
    "model_name": "mistralai/Mistral-7B-Instruct-v0.2",
    "layers_to_test": [25, 27],     # Testing L25 (Monster) and L27 (Classic)
    "early_layer": 5,
    "window_size": 16,
    "seed": 42,
    "n_pairs": 45,  # Target from Nov 16
...
# MAIN EXECUTION
# ==============================================================================
def run_nov16_reproduction():
    print("="*70)
    print("NOV 16-17 MISTRAL SINGULARITY REPRODUCTION")
    print("="*70)
    ...
    model = AutoModelForCausalLM.from_pretrained(
        CONFIG['model_name'],
        torch_dtype=torch.bfloat16,  # CRITICAL: bfloat16
        device_map="auto"
    )
    model.eval()
    ...
    for rec_group in recursive_groups:
        ...
        for base_group in baseline_groups:
            ...
            for i in range(min(len(rec_ids), len(base_ids))):
                # Check if baseline is long enough (>= window_size tokens)
                base_text = prompt_bank[base_ids[i]]["text"]
                base_tokens = tokenizer.encode(base_text)
                if len(base_tokens) >= CONFIG['window_size']:
    ...
    print("="*70)
    print(f"Valid pairs analyzed: {len(df) // len(CONFIG['layers_to_test'])}")
    ...
    for layer in CONFIG['layers_to_test']:
        ...
        if abs(d) > 3.0:
            print("  ⭐⭐⭐ SINGULARITY DETECTED (d > 3.0) ⭐⭐⭐")
        elif abs(d) > 2.0:
            print("  ⭐⭐ STRONG EFFECT (d > 2.0)")
        elif abs(d) > 0.8:
            print("  ✅ Moderate Effect (d > 0.8)")
        else:
            print("  ❌ No Effect")
    
    # Save results
    timestamp = pd.Timestamp.now().strftime("%Y%m%d_%H%M%S")
    filename = f"nov16_reproduction_{timestamp}.csv"
...
if __name__ == "__main__":
    results = run_nov16_reproduction()
