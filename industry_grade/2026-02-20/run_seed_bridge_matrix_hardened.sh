...
LOG_DIR="$REPO_ROOT/industry_grade/2026-02-20/evidence/seed_bridge_logs"
STATUS_FILE="$REPO_ROOT/industry_grade/2026-02-20/evidence/seed_bridge_status.csv"
...
capture_error_artifact() {
  ...
  latest_dir="$(ls -1dt "$run_root"/*"_${run_name}" 2>/dev/null | head -n 1 || true)"
  if [[ -n "$latest_dir" && -f "$latest_dir/error.txt" ]]; then
    cp "$latest_dir/error.txt" "$out_file"
  fi
}
...
while IFS=, read -r run_id condition seed config_path; do
  ...
  if already_succeeded "$run_id"; then
    echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] SKIP $run_id (already SUCCESS)"
    continue
  fi
...
echo "Completed hardened seed bridge matrix pass."
