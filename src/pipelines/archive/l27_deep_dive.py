...
def run_deep_dive(
    ...
    # Prompts must come from the sealed prompt bank (DEC15 hygiene: no hardcoded prompt strings)
    loader = PromptLoader()
    ...
    champion_prompt = str(loader.prompts[champion_id]["text"])
    baseline_prompt = str(loader.prompts[baseline_id]["text"])
    
    # Load dose-response prompts
    print("\n[2/6] Loading dose-response prompts...")
    dose_prompts: Dict[str, str] = {}
    dose_prompt_ids: Dict[str, str] = {}
    
    # Get prompts by level from the raw prompts dict (store ids for reproducibility)
    ...
    summary = {
        ...
        "prompt_ids": {
            ...
            "dose_response_levels": dose_prompt_ids,
        },
