...
OUTPUT_STEM="${OUTPUT_STEM:-induced_persistence_unselected_seed_v1}"
QUEUE_GROUP="${QUEUE_GROUP:-mistral_induced_persistence_unselected_seed_v1}"
EXPERIMENT_ID="${EXPERIMENT_ID:-induced_persistence_unselected_seed_v1}"
CLAIM_ID="${CLAIM_ID:-INDUCED_PERSISTENCE_UNSELECTED_SEED_V1}"
...
SESSIONS_PER_ARM="${SESSIONS_PER_ARM:-40}"
MAX_TURNS="${MAX_TURNS:-15}"
MAX_NEW_TOKENS="${MAX_NEW_TOKENS:-128}"
TEMPERATURE="${TEMPERATURE:-0.7}"
REP_PENALTY="${REP_PENALTY:-1.35}"
NOTES="${NOTES:-Five-arm unselected-seed persistence robustness test with fixed mixed prompt schedule.}"
...
"$PYTHON_BIN" -m src.utils.research_os result-upsert \
  ...
  --config-path "scripts/induced_persistence_unselected_seed_v1.py --source-run-dir $SOURCE_RUN_DIR --target-condition $TARGET_CONDITION --baseline-groups $BASELINE_GROUPS --sessions-per-arm $SESSIONS_PER_ARM --max-turns $MAX_TURNS" \
  --prompt-contract induced_seeded_followup_fixed_schedule \
  --metric-path "fixed_turn_schedule + self_feed_continuation + classify_output + compute_rv_with_components" \
  --claim-id "$CLAIM_ID"
