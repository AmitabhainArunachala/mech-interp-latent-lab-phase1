...
OUTPUT_STEM="${OUTPUT_STEM:-mistral_soft_break_tokenwindow_v1}"
QUEUE_GROUP="${QUEUE_GROUP:-mistral_soft_break_tokenwindow_v1}"
EXPERIMENT_ID="${EXPERIMENT_ID:-mistral_soft_break_tokenwindow_v1}"
CLAIM_ID="${CLAIM_ID:-MISTRAL_SOFT_BREAK_TOKENWINDOW_V1}"
NOTES="${NOTES:-Token-window localization for best anti-late soft break.}"
SCALE="${SCALE:?SCALE is required}"
TOKEN_WINDOWS="${TOKEN_WINDOWS:-1,2,4,8,16}"
...
run_step "$EXPERIMENT_ID" \
  "$PYTHON_BIN" scripts/mistral_soft_break_tokenwindow_v1.py \
  --device cuda \
  --out-dir "$RUN_OUT" \
...
"$PYTHON_BIN" -m src.utils.research_os result-upsert \
  ...
  --config-path "scripts/mistral_soft_break_tokenwindow_v1.py --scale $SCALE --token-windows $TOKEN_WINDOWS --max-new-tokens $MAX_NEW_TOKENS --temperature $TEMPERATURE --top-p $TOP_P" \
  --prompt-contract mistral_soft_break_tokenwindow \
  --metric-path "anti-late token-window localization on recursive and baseline prompts + BT_ART + MALFORMED" \
  --claim-id "$CLAIM_ID"
