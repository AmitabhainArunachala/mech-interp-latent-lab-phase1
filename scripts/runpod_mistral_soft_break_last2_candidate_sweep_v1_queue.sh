...
OUTPUT_STEM="${OUTPUT_STEM:-mistral_soft_break_last2_candidate_sweep_v1}"
QUEUE_GROUP="${QUEUE_GROUP:-mistral_soft_break_last2_candidate_sweep_v1}"
EXPERIMENT_ID="${EXPERIMENT_ID:-mistral_soft_break_last2_candidate_sweep_v1}"
CLAIM_ID="${CLAIM_ID:-MISTRAL_SOFT_BREAK_LAST2_CANDIDATE_SWEEP_V1}"
NOTES="${NOTES:-Narrow candidate sweep for last2 timed soft break.}"
TOKEN_WINDOW="${TOKEN_WINDOW:-2}"
SCALES="${SCALES:-0.5,0.75,1.0,1.25}"
CONDITION_NAMES="${CONDITION_NAMES:-anti_bridge_only,anti_l25_only,anti_l25_l27}"
...
run_step "$EXPERIMENT_ID" \
  "$PYTHON_BIN" scripts/mistral_soft_break_last2_candidate_sweep_v1.py \
  --device cuda \
  --out-dir "$RUN_OUT" \
...
"$PYTHON_BIN" -m src.utils.research_os result-upsert \
  ...
  --config-path "scripts/mistral_soft_break_last2_candidate_sweep_v1.py --token-window $TOKEN_WINDOW --scales $SCALES --condition-names $CONDITION_NAMES --max-new-tokens $MAX_NEW_TOKENS --temperature $TEMPERATURE --top-p $TOP_P" \
  --prompt-contract mistral_soft_break_last2_candidate_sweep \
  --metric-path "narrow candidate sweep for timed soft break on recursive and baseline prompts + BT_ART + MALFORMED" \
  --claim-id "$CLAIM_ID"
