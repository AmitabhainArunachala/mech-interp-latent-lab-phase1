...
OUTPUT_STEM="${OUTPUT_STEM:-mistral_caliper_matched_promptpass_v1}"
QUEUE_GROUP="${QUEUE_GROUP:-mistral_caliper_matched_promptpass_v1}"
EXPERIMENT_ID="${EXPERIMENT_ID:-mistral_caliper_matched_promptpass_v1}"
CLAIM_ID="${CLAIM_ID:-MISTRAL_CALIPER_MATCHED_PROMPTPASS_V1}"
NOTES="${NOTES:-Caliper-matched Mistral prompt-pass hardening on token length and prompt perplexity.}"
WINDOW="${WINDOW:-16}"
EARLY_LAYER="${EARLY_LAYER:-5}"
LATE_LAYER="${LATE_LAYER:-27}"
RECURSIVE_GROUPS="${RECURSIVE_GROUPS:-L5_refined,L4_full,L3_deeper}"
BASELINE_GROUPS="${BASELINE_GROUPS:-baseline_creative,baseline_math,long_control}"
...
run_step "$EXPERIMENT_ID" \
  "$PYTHON_BIN" scripts/mistral_caliper_matched_promptpass_v1.py \
  --device cuda \
  --output-dir "$RUN_OUT" \
  ...
  --recursive-groups "$RECURSIVE_GROUPS" \
...
"$PYTHON_BIN" -m src.utils.research_os result-upsert \
  ...
  --config-path "scripts/mistral_caliper_matched_promptpass_v1.py --window $WINDOW --early-layer $EARLY_LAYER --late-layer $LATE_LAYER --recursive-groups $RECURSIVE_GROUPS --baseline-groups $BASELINE_GROUPS" \
  --prompt-contract mistral_caliper_matched_promptpass \
  --metric-path "prompt-pass R_V + prompt perplexity + token-length caliper matching" \
  --claim-id "$CLAIM_ID"
